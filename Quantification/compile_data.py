...
def region_prop_table_filter(cdf, t_dict):
    ...
    for prop_name, (min_value, max_value) in t_dict.items():
        ...
        filter_counts_post_thresh[f'post_{prop_name}'] = get_colocal_id_counts(cdf)
...
def drop_duplicate_colocalizations(rpdf_colocal, intersecting_label_column, group_by_col='img_name'):
    ...
    if group_by_col is None:
        dropped_dupes = (rpdf_colocal
         .sort_values([intersecting_label_column, 'intersection_p'], ascending=[True, False])
         .drop_duplicates(subset=[intersecting_label_column], keep='first'))
    else:
        dropped_dupes = (rpdf_colocal
                .groupby(group_by_col, as_index=False)
                .apply(lambda x: x.sort_values(by=[intersecting_label_column, 'intersection_p'], ascending=[True, False])
                            .drop_duplicates(subset=[intersecting_label_column], keep='first'))
                )
    try:
...
def get_colocal_id_counts(rpdf, num_colocal_ids=4):
    vc = rpdf.value_counts('colocal_id').to_dict()
    return {i:0 if i not in vc else vc[i] for i in range(num_colocal_ids)}


def get_nuclei_per_region_df(rpdf, region_df, ont, geojson_path, count_channel_names=['nDapi',	'nZif',	'nGFP',	'nBoth']):
    """
    Params:
...
def inspect_output(all_data_df, inspect_region_id=295, 
                   ...
                   df_calcs = {'reactivation': lambda df: df['nBoth']/df['nGFP'], 'groupcohort': lambda df: df['group']+df['cohort']},
                   sig_diff_groups=['FC', 'FC+EXT'], sig_diff_col = 'reactivation', 
                   summary_df_cols = ['groupcohort', 'animal_id', 'nGFP', 'nZif', 'nBoth', 'reactivation'],
    ):
    # get nan counts 
...
class Dispatcher:
    ...
    def run(self):
        ...
        for datum in datums:
            ...
            counts_df = get_nuclei_per_region_df(rpdf_filtered, region_df, ont, datum.geojson_regions_dir_paths, count_channel_names=self.count_channel_names)
...
def rpdf_only_worker(disp):
    try:
        return disp.rpdf_compile()
    except Exception as e:
        print(f"An error occurred: {e}")
        return pd.DataFrame()  # Return an empty DataFrame on error

def run_parallel_rpdf_concat(disps):
    with concurrent.futures.ProcessPoolExecutor() as executor:
        executor.map(rpdf_only_worker, disps)
    return 0
...
def get_dispatchers(
        ...
        THRESHOLD_DICTS, outdir, THE_DATE, MODE, TEST=False
    ):
    dispatchers = []
    for an in animals:
