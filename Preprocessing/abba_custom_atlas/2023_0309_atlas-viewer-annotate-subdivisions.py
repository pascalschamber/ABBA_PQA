...
####################################################################################
# 03/13/23
# compare og and new labels from their respective .h5 files
# point is to understand what I need to change in the ontology to update with the new subdivisions
# 303 is px of BLAa previously, which corresponds to its id
viewer = napari.Viewer() # create napari viewer

# old labels
img_path_atlas_og = r'F:\ABBA\abba_atlases\ccf2017-mod65000-border-centered-mm-bc.h5'
atlas_h5_og = h5py.File(img_path_atlas_og)
og_h5_labels = np.array(atlas_h5_og['t00000']['s03']['0']['cells'])
og_h5_labels = np.moveaxis(og_h5_labels, [0, 2], [2,0])
viewer.add_image(og_h5_labels, name='og_atlas_labels')

# new labels
img_path_atlas_new = r'D:\ReijmersLab\TEL\ccf2017_atlas_images\ccf2017-mod65000-border-centered-mm-bc.h5'
atlas_h5_new = h5py.File(img_path_atlas_new)
new_h5_labels = np.array(atlas_h5_new['t00000']['s03']['0']['cells'])
new_h5_labels = np.moveaxis(new_h5_labels, [0, 2], [2,0])
viewer.add_image(new_h5_labels, name='final_atlas_labels')
####################################################################################


####################################################################################
# 03/09/23
# only edited to try to figure out how to load my anno into abba
# code below did not work
# import cv2
# from pathlib import Path
# atlas_dir = r'D:\ReijmersLab\TEL\ccf2017_atlas_images'
# p1 = 'ccf2017_nissel.tif'
# p2 = 'ccf2017_labels_bla-subdivisions_20230302.tif'
# p1 = os.path.join(atlas_dir, p1)
# p2 = os.path.join(atlas_dir, p2)
# img1 , img2 = imread(p1), imread(p2)

# # <HDF5 dataset "cells": shape (1140, 800, 1320), type "<i2">
#     # current
#     # (1320, 800, 1140) int16 2 27433 0
#     # (1320, 800, 1140) int32 4 30002 -23066

# img1, img2 = np.moveaxis(img1, [0, 2], [2,0]), np.moveaxis(img2, [0, 2], [2,0])
# imgs = [img1, img2]
# outnames = ['ccf2017_nissel_2.tif','ccf2017_labels_bla-subdivisions_20230302_2.tif']
# for i in range(len(imgs)):
#     img = imgs[i]
#     print(
#         img.shape, img.dtype, img.itemsize, img.max(), img.min()
#     )
#     if bool(0):
#         skimage.io.imsave( 
#                     os.path.join(atlas_dir, outnames[i]), img)


# cv2.imwrite(os.path.join(atlas_dir, 'ccf2017_labels_bla-subdivisions_16bit_20230302.tif')
####################################################################################
