...
def main(in_paths, out_path, px_size=0.650, is_mask=False):
...
#     if bool(0):
#         # prepare an image for a demo
#         # by spliting the channels
#         from pathlib import Path
#         image_path_dir = r'D:\ReijmersLab\TEL\slides\processed_czi_images\fullsize\TEL15'
#         image_channels_outdir = r'D:\ReijmersLab\TEL\slides\processed_czi_images\test_generate_image_pyramids'
#         image_paths = [os.path.join(image_path_dir, c) for c in os.listdir(image_path_dir)]
#         for image_path in image_paths[:3]:
#             image = cv2.imread(image_path)
#             fn = Path(image_path).stem
#             for i in range(3):
#                 image_outpath = os.path.join(image_channels_outdir, f'{fn}_ch{i}.tif')
#                 ch = image[...,i]
#                 print(ch.max())
#                 tifffile.imwrite(image_outpath, ch)
#         px_size_of_czi_file = 0.650 # this is important to display properly in abba, so make sure flag is set when creating ome.tiff
        
#     # main(in_paths, out_path, px_size=0.650, is_mask=False)
